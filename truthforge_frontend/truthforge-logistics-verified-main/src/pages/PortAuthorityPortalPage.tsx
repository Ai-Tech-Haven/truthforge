...
import { mockShipments, mockPortTrustReceipts, ShipmentTracking, PortTrustReceipt as PortTrustReceiptType } from "@/lib/mock-data";
...
import {
  ...
  Flag, X, ChevronDown, ChevronUp, Anchor, Plane, Truck, RefreshCw,
} from "lucide-react";
...
const PortAuthorityPortalPage = () => {
  ...
  const fetchLiveData = useCallback(async () => {
    ...
    try {
      const [sqRes, prRes] = await Promise.allSettled([
        fetch(`${RAILWAY}/api/clearance/queue`, { signal: AbortSignal.timeout(8000) }),
        fetch(`${RAILWAY}/api/port-trust-receipts`, { signal: AbortSignal.timeout(8000) }),
      ]);
      if (sqRes.status === "fulfilled" && sqRes.value.ok) {
        const d = await sqRes.value.json();
        setLiveShipments(d.shipments ?? []);
      }
      if (prRes.status === "fulfilled" && prRes.value.ok) {
        const d = await prRes.value.json();
        setLiveReceipts(d.receipts ?? d ?? []);
      }
    } catch { /* keep empty */ }
  ...
  useEffect(() => {
    if (!isMockMode) fetchLiveData();
  }, [isMockMode, fetchLiveData]);

  const shipments = isMockMode ? mockShipments : (liveShipments.length > 0 ? liveShipments : mockShipments);
  const receipts  = isMockMode ? mockPortTrustReceipts : (liveReceipts.length > 0 ? liveReceipts : mockPortTrustReceipts);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {!isMockMode && (
            <button onClick={fetchLiveData} className="flex items-center gap-1 text-xs text-accent hover:underline" aria-label="Refresh">
              <RefreshCw className={`h-3.5 w-3.5 ${shipmentsLoading ? "animate-spin" : ""}`} />
              {shipmentsLoading ? "Loading..." : "Refresh"}
            </button>
          )}
          <div className="relative">
            <button
              ...
              aria-haspopup="listbox" aria-expanded={portOpen}
            >
            ...
            {portOpen && (
              <div className="absolute right-0 mt-1 w-48 rounded-lg border border-border bg-card shadow-lg z-20" role="listbox">
                {PORTS.map((p) => (
                  <button key={p.code} role="option" aria-selected={selectedPort === p.code}
                    onClick={() => { setSelectedPort(p.code); setPortOpen(false); }}
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-accent/10 transition-colors first:rounded-t-lg last:rounded-b-lg ${selectedPort === p.code ? "text-accent font-semibold" : "text-foreground"}`}>
                    {p.label} ({p.code})
                  </button>
                ))}
              </div>
      ...
      <div className="flex items-center gap-1 p-1 rounded-lg bg-secondary w-fit">
        {MODES.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => { setTransportMode(key); setExpandedRow(null); }}
            className={`flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${transportMode === key ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
      ...
      <div className="rounded-xl border border-border bg-card shadow-card overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <div className="p-8 text-center text-sm text-muted-foreground">
            {shipmentsLoading ? "Loading live shipments..." : `No ${transportMode} shipments in queue.`}
          </div>
        ) : (
                  ...
                  <div className="p-4 hover:bg-accent/5 transition-colors">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="hidden md:block">
                        {txRef && topicId ? (
                          <a href={`https://hashscan.io/testnet/account/${topicId}`} target="_blank" rel="noopener noreferrer"
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 bg-secondary/30 border-t border-border/60 space-y-4">
                      <div className="rounded-lg border border-border bg-card p-3 mt-3">
                        <div className="text-[10px] font-heading font-bold text-muted-foreground uppercase tracking-wider mb-2">{labels.batch} Intelligence</div>
                      ...
                      {containers.length > 0 && (
                        <div className="rounded-lg border border-border bg-card overflow-hidden">
                          <div className="text-[10px] font-heading font-bold text-muted-foreground uppercase tracking-wider px-3 pt-3 pb-2">{labels.unit} Intelligence</div>
                              ...
                              <tbody>
                                {containers.slice(0, 8).map((c) => (
                                  <tr key={c.id} className="border-b border-border last:border-0">
                                    ...
                                    <td className="py-1.5 px-3">
                                      <span className={`text-[10px] font-medium ${c.riskLevel === "high" ? "text-destructive" : c.riskLevel === "medium" ? "text-warning" : "text-success"}`}>
                          ...
                          <div className="px-3 pb-3 pt-2">
                            <div className="text-[10px] text-muted-foreground mb-1.5">{labels.unit} Grid</div>
                            <div className="flex flex-wrap gap-1">
                              {containers.map((c) => (
                                <span key={c.id} title={`${c.id} - ${c.status}`} className="text-base leading-none" aria-label={`${c.id} ${c.status}`}>
                        ...
                        <div className="rounded-lg border border-border bg-card p-3">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                            <div>
                              ...
                              <span className={`font-medium ${receipt.clearanceStatus === "cleared" ? "text-success" : receipt.clearanceStatus === "flagged" ? "text-destructive" : "text-warning"}`}>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {isMockMode ? (
                                ...
                                <a href={`https://hashscan.io/testnet/account/${receipt.hederaTxRef.split("@")[0]}`}
