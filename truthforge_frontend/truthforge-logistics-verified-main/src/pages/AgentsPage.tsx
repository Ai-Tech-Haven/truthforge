...
const AgentsPage = () => {
  ...
  useEffect(() => {
    ...
    apiFetch<{ agents: LiveAgent[] }>("/api/agents")
      .then((data) => {
        if (!cancelled) setAgents(data.agents as typeof mockAgents);
      })
      .catch((err) => {
        if (cancelled) return;
        ...
        <div className="flex items-center gap-3 mt-1 flex-wrap">
          ...
          {loading && (
            <span className="flex items-center gap-1 text-xs text-accent">
              <RefreshCw className="h-3 w-3 animate-spin" /> Syncing live data…
            </span>
          )}
          {!isMockMode && !loading && !error && (
            <span className="text-xs font-heading font-bold text-success bg-success/10 border border-success/20 px-2 py-0.5 rounded uppercase tracking-wider">
              Live
            </span>
          )}
        </div>
        {error && (
          <p className="text-xs text-warning mt-1">{error}</p>
        )}
